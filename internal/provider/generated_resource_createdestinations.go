...
func init() {
	RegisterGeneratedResource(NewCreatedestinationsResource)
}

// CreatedestinationsResource implements the createdestinations resource
type CreatedestinationsResource struct {
	client *apiClient
}

// createdestinationsModel represents the resource data model
type createdestinationsModel struct {
...
// NewCreatedestinationsResource creates a new createdestinations resource
func NewCreatedestinationsResource() resource.Resource {
	return &CreatedestinationsResource{}
}

// Metadata returns the resource type name
func (r *CreatedestinationsResource) Metadata(_ context.Context, req resource.MetadataRequest, resp *resource.MetadataResponse) {
	resp.TypeName = "umbrella_createdestinations"
}

// Configure configures the resource with the provider client
func (r *CreatedestinationsResource) Configure(_ context.Context, req resource.ConfigureRequest, resp *resource.ConfigureResponse) {
...
func (r *CreatedestinationsResource) Schema(_ context.Context, _ resource.SchemaRequest, resp *resource.SchemaResponse) {
	resp.Schema = schema.Schema{
		Description: "createdestinations resource",
		Attributes: map[string]schema.Attribute{
			"id":                          schema.StringAttribute{Computed: true, Description: "Resource identifier"},
			"destination":                 schema.StringAttribute{Required: true, Description: "A domain, URL, or IP."},
			"comment":                     schema.StringAttribute{Optional: true, Description: "A comment about the destination."},
...
// Create creates a new createdestinations
func (r *CreatedestinationsResource) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	var plan createdestinationsModel
	...
	if !plan.Destination.IsNull() {
		requestBody["destination"] = plan.Destination.ValueString()
	}
	if !plan.Comment.IsNull() {
		requestBody["comment"] = plan.Comment.ValueString()
	}

	// Make API call
	result, err := r.client.CreateResource(ctx, "/destinationlists/{destinationListId}/destinations", requestBody)
	if err != nil {
		resp.Diagnostics.AddError("Client Error", fmt.Sprintf("Unable to create createdestinations, got error: %s", err))
		return
	}
	...
	if result.Data != nil {
		if dataMap, ok := result.Data.(map[string]interface{}); ok {
			...
			if val, exists := dataMap["destination"]; exists && val != nil {
				if strVal, ok := val.(string); ok {
					plan.Destination = types.StringValue(strVal)
				}
			}
			if val, exists := dataMap["comment"]; exists && val != nil {
				if strVal, ok := val.(string); ok {
					plan.Comment = types.StringValue(strVal)
				}
			}
...
// Read reads the createdestinations
func (r *CreatedestinationsResource) Read(ctx context.Context, req resource.ReadRequest, resp *resource.ReadResponse) {
	var state createdestinationsModel
...
// Update updates the createdestinations
func (r *CreatedestinationsResource) Update(ctx context.Context, req resource.UpdateRequest, resp *resource.UpdateResponse) {
	var plan createdestinationsModel
	resp.Diagnostics.Append(req.Plan.Get(ctx, &plan)...)
	if resp.Diagnostics.HasError() {
		return
	...
	resp.Diagnostics.AddError("Configuration Error", "No update endpoint configured for createdestinations")
	return

	resp.Diagnostics.Append(resp.State.Set(ctx, plan)...)
}

// Delete deletes the createdestinations
func (r *CreatedestinationsResource) Delete(ctx context.Context, req resource.DeleteRequest, resp *resource.DeleteResponse) {
	var state createdestinationsModel
	resp.Diagnostics.Append(req.State.Get(ctx, &state)...)
	if resp.Diagnostics.HasError() {
		return
	...
	resp.Diagnostics.AddError("Configuration Error", "No delete endpoint configured for createdestinations")
}
